    ...
    <div className="shrink-0 border-t border-line bg-surface px-4 pb-4 pt-3 sm:px-6">
      ...
      <form onSubmit={handleSubmit} className="rounded-xl border border-line bg-[#fbfcfd] p-2 shadow-hairline transition focus-within:border-brand/50 focus-within:ring-2 focus-within:ring-brand/10">
        ...
        <textarea
          ...
          placeholder={disabled ? "Create a session to get started..." : `Ask ${agentName} to analyze your workspace...`}
          rows={2}
        ...
        <div className="flex items-center justify-between gap-2 px-1 pt-1">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Button type="submit" variant="primary" size="icon" className="h-9 w-9 rounded-lg" disabled={disabled || isProcessing || draft.trim().length === 0} aria-label={disabled ? "Create a session first" : isProcessing ? `${agentName} is working` : "Send message"} title={disabled ? "Create a session first" : isProcessing ? `${agentName} is working` : "Send message"}>
