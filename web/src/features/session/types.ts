...
export type TurnUsage = { inputTokens: number; outputTokens: number; totalTokens: number };
export type ToolDisplayInfo = { title: string; subject?: string; detail?: string };
export type AgentExecutionStep = { id: string; callId: string; name: string; status: TurnResponseBlockStatus; display?: ToolDisplayInfo; startedAt?: string; completedAt?: string };
...
export type GeneratedArtifact = { id: string; resourceId: string; name: string; detail: string; size: string; kind: AttachmentKind; generatedAt: string };
