...
import { ModelRetryIndicator } from "./components/model-retry-indicator";
...
type RightPanelMode = "context" | "trace";
...
export function SessionPage() {
  ...
  const [rightPanelMode, setRightPanelMode] = useState<RightPanelMode>("context");
  ...
  const traceQuery = useTurnTrace(
    rightPanelMode === "trace" ? activeSessionId : null,
    rightPanelMode === "trace" ? selectedTraceTurnId : null,
    accessToken,
  ...
  async function consumeTurnStream(sessionId: string, expectedTurnId: string | undefined, stream: AsyncGenerator<TurnStreamEvent>, controller: AbortController, allowRecovery: boolean, optimisticMessageId?: string) {
    ...
    try {
      for await (const event of stream) {
        ...
        if (event.type === "model_retry") setSessionStatus(sessionId, `Retrying model · attempt ${event.nextAttempt}`);
  ...
  function handleSessionSelect(sessionId: string) { if (!sessions.some((item) => item.id === sessionId)) return; setActiveSessionId(sessionId); setDraft(""); setIsMobileNavVisible(false); setRightPanelMode("context"); setSelectedTraceTurnId(null); setIsContextVisible(false); }
  ...
  function handleOpenTrace(turnId: string) { setSelectedTraceTurnId(turnId); setRightPanelMode("trace"); setIsContextVisible(true); }
  function handleToggleContext() { setRightPanelMode("context"); setIsContextVisible((current) => !current); }
  function handleBackToContext() { setRightPanelMode("context"); }
  function handleCloseRightPanel() { setIsContextVisible(false); }
  ...
  const isRetryingModel = activeState?.retry !== null && activeState?.retry !== undefined;
  ...
  return <div className="min-h-dvh bg-canvas text-ink"><a href="#main-content" className="sr-only focus:not-sr-only focus:fixed focus:left-4 focus:top-4 focus:z-50 focus:rounded-lg focus:bg-ink focus:px-4 focus:py-3 focus:text-sm focus:text-white">Skip to session</a><div className="flex min-h-dvh">
    ...
    <main className="flex min-w-0 flex-1 flex-col" id="main-content" tabIndex={-1}><div className="flex h-14 shrink-0 items-center border-b border-line bg-surface px-4 lg:hidden"><button type="button" onClick={() => setIsMobileNavVisible(true)} className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-lg text-mutedInk" aria-label="Open session history"><Menu size={18} /></button><span className="ml-2.5 text-sm font-bold tracking-[-0.02em]">opspilot</span></div>
      <SessionHeader title={activeSession?.title ?? "New session"} subtitle={isRetryingModel ? "Retrying model request" : isProcessing ? activeState?.compaction.status === "running" ? "Optimizing context" : activeState?.isThinking ? "Thinking" : "Processing request" : activeSession ? "Ready" : "Create a Session to begin"} agentName={demoAgentName} statusLabel={isRetryingModel ? "Retrying" : isProcessing ? "Processing" : "Ready"} onToggleContext={handleToggleContext} isContextVisible={isContextVisible} />
      <div className="flex min-h-0 flex-1"><section className="flex min-w-0 flex-1 flex-col"><div className="flex-1 overflow-y-auto scroll-smooth"><div className="mx-auto w-full max-w-[920px] px-4 pb-8 pt-6 sm:px-8 sm:pt-8"><div className="mb-7 flex items-center justify-between gap-4"><div><p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-mutedInk">Session</p><p className="mt-1 text-xs text-mutedInk">{activeSession ? formatSessionUpdatedAt(activeSession.updatedAt) : "No active session"}</p></div></div>{visibleError && <div role="alert" className="mb-6 flex items-start gap-2.5 rounded-lg border border-danger/25 bg-danger/[0.06] px-3.5 py-3 text-sm text-danger"><AlertCircle size={17} className="mt-0.5 shrink-0" /><p>{visibleError}</p></div>}<SessionThread items={timeline} agentName={demoAgentName} onOpenTrace={handleOpenTrace} sessionId={activeSessionId ?? undefined} accessToken={accessToken} onArtifactDownloadError={(message) => { if (activeSessionId) setErrorsBySessionId((current) => ({ ...current, [activeSessionId]: message })); }} /><ModelRetryIndicator retry={activeState?.retry} /></div></div><Composer disabled={!activeSessionId || loadingSessionIds[activeSessionId ?? ""] === true} draft={draft} attachments={attachments} isProcessing={isProcessing} agentName={demoAgentName} onDraftChange={setDraft} onSubmit={handleSubmit} onAttach={handleAttach} onRemoveAttachment={handleRemoveAttachment} /></section>{rightPanelMode === "trace" && selectedTraceTurnId !== null && activeSessionId !== null ? <TracePanel turnId={selectedTraceTurnId} trace={traceQuery.trace} isLoading={traceQuery.isLoading} error={traceQuery.error} onRefresh={traceQuery.refresh} onBack={handleBackToContext} /> : <ContextPanel status={demoContextStatus} files={demoContextFiles} tools={demoConnectedTools} outputs={demoRecentOutputs} />}</div>
    ...
    {isContextVisible && <div className="fixed inset-0 z-30 bg-navy/20 xl:hidden" onClick={handleCloseRightPanel}><div className="ml-auto h-full max-w-[360px]" onClick={(event) => event.stopPropagation()}>{rightPanelMode === "trace" && selectedTraceTurnId !== null && activeSessionId !== null ? <TracePanel isMobile turnId={selectedTraceTurnId} trace={traceQuery.trace} isLoading={traceQuery.isLoading} error={traceQuery.error} onRefresh={traceQuery.refresh} onBack={handleBackToContext} onClose={handleCloseRightPanel} /> : <ContextPanel isMobile status={demoContextStatus} files={demoContextFiles} tools={demoConnectedTools} outputs={demoRecentOutputs} onClose={handleCloseRightPanel} />}</div></div>}
  </div>;
}
