...
describe("TurnStreamState", () => {
  it("hydrates partial assistant text and tools from projection", () => {
    const state = hydrateTurnStreamStateFromProjection({ ...identity, status: "running", startedAt: "2026-09-09T00:00:00Z", assistant: { text: "partial", messageVisible: true, isThinking: false }, tools: [{ callId: "call-1", name: "lookup", status: "running", display: { title: "Look up", subject: "record-1" }, startedAt: "2026-09-09T00:00:01Z", completedAt: "2026-09-09T00:00:02Z" }], compaction: { status: "idle" }, usage: { inputTokens: 10, outputTokens: 20, totalTokens: 30 }, retry: null, lastSequence: 7 });
  ...
  it("reduces live events by Turn identity and sequence", () => {
    let state = hydrateTurnStreamStateFromProjection({ ...identity, status: "running", assistant: { text: "", messageVisible: false, isThinking: false }, tools: [], compaction: { status: "idle" }, usage: null, retry: null, lastSequence: -1 });
  ...
  it("hydrates and clears retry projection state on real model output", () => {
    let state = hydrateTurnStreamStateFromProjection({ ...identity, status: "running", assistant: { text: "", messageVisible: false, isThinking: false }, tools: [], compaction: { status: "idle" }, usage: null, retry: { modelCallId: "model-1", failedAttempt: 1, nextAttempt: 2, delayMs: 500, kind: "timeout" }, lastSequence: 3 });
    expect(state.retry).toMatchObject({ modelCallId: "model-1", nextAttempt: 2, kind: "timeout" });

    state = reduceTurnStreamEvent(state, { ...identity, type: "assistant_thinking_started", sequence: 4, timestamp: "2026-09-09T00:00:04Z" });
    expect(state.retry).toBeNull();
  });

  it("tracks a live retry and clears it when the turn terminates", () => {
    let state = hydrateTurnStreamStateFromProjection({ ...identity, status: "running", assistant: { text: "", messageVisible: false, isThinking: false }, tools: [], compaction: { status: "idle" }, usage: null, retry: null, lastSequence: -1 });
    state = reduceTurnStreamEvent(state, { ...identity, type: "model_retry", modelCallId: "model-1", failedAttempt: 1, nextAttempt: 2, delayMs: 500, kind: "rate_limit", sequence: 0, timestamp: "2026-09-09T00:00:00Z" });
    expect(state.retry).toMatchObject({ failedAttempt: 1, nextAttempt: 2, kind: "rate_limit" });

    state = reduceTurnStreamEvent(state, { ...identity, type: "turn_failed", message: "failed", sequence: 1, timestamp: "2026-09-09T00:00:01Z" });
    expect(state.retry).toBeNull();
  });
});
