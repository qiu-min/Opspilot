...
describe("TurnStreamState", () => {
  it("hydrates partial assistant text and tools from projection", () => {
    const state = hydrateTurnStreamStateFromProjection({ ...identity, status: "running", assistant: { text: "partial", messageVisible: true, isThinking: false }, tools: [{ callId: "call-1", name: "lookup", status: "running", display: { title: "Look up", subject: "record-1" } }], compaction: { status: "idle" }, usage: null, lastSequence: 7 });
    expect(state.lastSequence).toBe(7); expect(state.assistantMessages[0].text).toBe("partial"); expect(state.toolExecutions[0]).toMatchObject({ callId: "call-1", display: { title: "Look up", subject: "record-1" } });
  });
  ...
  it("preserves tool display metadata when a later lifecycle event omits it", () => {
    ...
    expect(state.toolExecutions[0]).toMatchObject({ status: "completed", display: { title: "Inspect Worksheet", subject: "Sheet1" } });
  });
});
