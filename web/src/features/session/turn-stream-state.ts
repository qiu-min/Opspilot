...
import type { ModelFailureKind, ToolDisplayInfo, TurnStreamEvent } from "../../api/sessions/turn-stream-contracts";
...
export type TurnStreamRetry = { modelCallId: string; failedAttempt: number; nextAttempt: number; delayMs: number; kind: ModelFailureKind };
...
export function createInitialTurnStreamState(turnId = "", sessionId = ""): TurnStreamState {
  return { turnId, sessionId, phase: "idle", lastSequence: -1, isThinking: false, assistantMessages: [], activity: [], toolExecutions: [], usageEvents: [], retry: null, compaction: { status: "idle", reason: null, aborted: false, failed: false, willRetry: false }, completion: null, error: null };
}
...
export function reduceTurnStreamEvent(state: TurnStreamState, event: TurnStreamEvent): TurnStreamState {
  ...
  switch (event.type) {
    ...
    case "assistant_thinking_started": return { ...next, phase: "streaming", isThinking: true, retry: null };
    case "assistant_thinking_completed": return { ...next, isThinking: false };
    case "assistant_message_started": return { ...next, isThinking: false, retry: null, assistantMessages: [...next.assistantMessages, { text: "", completed: false }], activity: [...next.activity, { type: "assistant-message", messageIndex: next.assistantMessages.length }] };
    case "assistant_text_delta": {
      ...
      return { ...next, phase: "streaming", isThinking: false, retry: null, assistantMessages: messages };
    }
    case "assistant_message_completed": {
      const index = findActiveAssistant(next);
      const messages = [...next.assistantMessages];
      messages[index] = { ...messages[index], completed: true };
      return { ...next, assistantMessages: messages, isThinking: false, retry: null };
    }
    case "tool_queued": return updateTool({ ...next, retry: null }, { batchId: event.batchId ?? `tool-batch-${event.callId}`, callId: event.callId, name: event.name, status: "queued", display: event.display });
    ...
    case "model_retry": return { ...next, phase: "streaming", retry: { modelCallId: event.modelCallId, failedAttempt: event.failedAttempt, nextAttempt: event.nextAttempt, delayMs: event.delayMs, kind: event.kind } };
    case "turn_completed": return { ...next, phase: "completed", isThinking: false, retry: null, completedAt: next.completedAt ?? event.timestamp, completion: { sessionId: event.sessionId, turnId: event.turnId, leafId: event.resultLeafId, status: "completed" } };
    case "turn_failed": return { ...next, phase: "error", isThinking: false, retry: null, completedAt: next.completedAt ?? event.timestamp, completion: { sessionId: event.sessionId, turnId: event.turnId, leafId: null, status: "failed" }, error: event.message };
    case "turn_cancelled": return { ...next, phase: "cancelled", isThinking: false, retry: null, completedAt: next.completedAt ?? event.timestamp, completion: { sessionId: event.sessionId, turnId: event.turnId, leafId: null, status: "cancelled" } };
  }
}
