...
export function reduceTurnStreamEvent(state: TurnStreamState, event: TurnStreamEvent): TurnStreamState {
  ...
  switch (event.type) {
    case "turn_started": return { ...next, phase: "streaming", error: null };
    ...
    case "turn_completed": return { ...next, phase: "completed", isThinking: false, completion: { sessionId: event.sessionId, turnId: event.turnId, leafId: event.resultLeafId, status: "completed" } };
    case "turn_failed": return { ...next, phase: "error", isThinking: false, completion: { sessionId: event.sessionId, turnId: event.turnId, leafId: null, status: "failed" }, error: event.message };
    case "turn_cancelled": return { ...next, phase: "cancelled", isThinking: false, completion: { sessionId: event.sessionId, turnId: event.turnId, leafId: null, status: "cancelled" } };
  }
}
